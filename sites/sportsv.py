...
# 設定日誌
logger = logging.getLogger("sportsv_crawler")

class Crawler(BaseCrawler):
    """運動視界爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化東森電視爬蟲"""
        super().__init__("sportsv")
        logger.info("運動視界初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            ...
            if "content" in article:
                ...
                logger.info(f"運動視界處理完成: {url}, 長度: {len(article['content'])}")
