...
# 設定日誌
logger = logging.getLogger("businessToday_crawler")

class Crawler(BaseCrawler):
    """今週刊爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化今週刊爬蟲"""
        super().__init__("businessToday")
        logger.info("今週刊爬蟲初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            if "content" in article:
                ...
                logger.info(f"今週刊文章處理完成: {url}, 長度: {len(article['content'])}")
