...
# 設定日誌
logger = logging.getLogger("chinatimes_crawler")

class Crawler(BaseCrawler):
    """中時爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化中時爬蟲"""
        super().__init__("chinatimes")
        logger.info("中時爬蟲初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            if "content" in article:
                ...
                logger.info(f"中時文章處理完成: {url}, 長度: {len(article['content'])}")
