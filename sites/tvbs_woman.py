...
# 設定日誌
logger = logging.getLogger("tvbs_woman_crawler")

class TvbsCrawler(BaseCrawler):
    """TVBS女人我最大爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化 TVBS 爬蟲"""
        super().__init__("tvbs_woman")
        logger.info("TVBS女人我最大初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            ...
            if "content" in article:
                ...
                # 提取新聞類別
                category_match = re.search(r"https://woman\.tvbs\.com\.tw/([^/]+)/", url)
                if category_match:
                    article["category"] = category_match.group(1)
                
                logger.info(f"TVBS女人我最大處理完成: {url}, 長度: {len(article['content'])}")
