...
# 設定日誌
logger = logging.getLogger("thenewslens_crawler")

class Crawler(BaseCrawler):
    """關鍵評論網爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化關鍵評論網爬蟲"""
        super().__init__("thenewslens")
        logger.info("關鍵評論網爬蟲初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            if "content" in article:
                ...
                logger.info(f"關鍵評論網文章處理完成: {url}, 長度: {len(article['content'])}")
