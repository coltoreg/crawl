...
# 設定日誌
logger = logging.getLogger("ftvtv_crawler")

class Crawler(BaseCrawler):
    """民視新聞網爬蟲，繼承並擴展 BaseCrawler"""
    
    def __init__(self):
        """初始化民視新聞網爬蟲"""
        super().__init__("ftvtv")
        logger.info("民視新聞網爬蟲初始化完成")
    ...
    async def extract_article(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        if article:
            if "content" in article:
                ...
                logger.info(f"民視新聞網文章處理完成: {url}, 長度: {len(article['content'])}")
